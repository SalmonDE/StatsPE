...
class UpdaterTask extends PluginTask
{
    ...
    public function onCompletion(Server $server){
        if($this->getResult){
            $server->broadcastMessage(TF::RED.TF::BOLD.'Restarting Server due to a software update!');
            $server->broadcastTip(TF::RED.TF::BOLD.'Restarting Server due to a software update!');
            sleep(2);
            $server->shutdown();
        }
    }
